...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // TODO: student need to change the next line from output[i] = source[i]
    ...
    // set output to XOR cipher algorithm
...
std::string read_file(const std::string& filename)
{
  ...
  // TODO: implement loading the file into a string
    ...
    // create input file stream readFile using passed filename
    std::ifstream readFile(filename);

    // assign data read from file to string file_text to be returned
    file_text.assign((std::istreambuf_iterator<char>(readFile)), (std::istreambuf_iterator<char>()));

    // close readFile stream
    readFile.close();   
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    // create output file stream saveFile 
    std::ofstream saveFile;
    // set the output file to the filename received
    saveFile.open(filename);

    // using localtime_s to pull current time to assist in building timestamp
    struct tm currTime;
    time_t now = time(0);
    localtime_s(&currTime, &now);
    
    // pulling year, month and day for timestamp
    int year = 1900 + currTime.tm_year;
    int month = 1 + currTime.tm_mon;
    int day = currTime.tm_mday;

    // write the student name, timestamp, key and data to the file
    saveFile << student_name << std::endl;
    saveFile << year << "-" << month << "-" << day << std::endl;
    saveFile << key << std::endl;
    saveFile << data;

    // close saveFile stream
    saveFile.close();
}
